...
create table if not exists public.directory_removal_requests (
  id bigint generated by default as identity primary key,
  contact_channel text not null check (contact_channel in ('whatsapp', 'messenger', 'facebook')),
...
create table if not exists public.provider_contact_reports (
  id bigint generated by default as identity primary key,
  provider_contact_id bigint not null references public.provider_contacts(id) on delete cascade,
  reporter_id uuid not null references public.profiles(id) on delete cascade,
  reporter_role text not null check (reporter_role in ('reviewer', 'tester', 'admin')),
  report_type text not null check (report_type in ('no_reply', 'not_provider', 'trusted', 'scam', 'broken_contact')),
  status text not null default 'open' check (status in ('open', 'in_review', 'resolved', 'dismissed')),
  admin_note text,
  created_at timestamptz default now(),
  updated_at timestamptz default now(),
  unique (provider_contact_id, reporter_id, report_type)
);
...
alter table public.provider_contact_reports enable row level security;
...
create policy "directory removal requests admin read"
on public.directory_removal_requests
for select
using (public.is_admin(auth.uid()));

create policy "directory removal requests admin update"
on public.directory_removal_requests
for update
using (public.is_admin(auth.uid()))
with check (public.is_admin(auth.uid()));

create policy "users create provider contact reports"
on public.provider_contact_reports
for insert
with check (
  ...
  and exists (
    ...
    from public.profiles
    where id = auth.uid()
      and role in ('reviewer', 'tester', 'admin')
  )
);

create policy "users update own provider contact reports"
on public.provider_contact_reports
for update
using (auth.uid() = reporter_id or public.is_admin(auth.uid()))
with check (auth.uid() = reporter_id or public.is_admin(auth.uid()));

create policy "admins read provider contact reports"
on public.provider_contact_reports
for select
using (public.is_admin(auth.uid()));
