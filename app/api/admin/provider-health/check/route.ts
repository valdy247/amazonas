...
export async function POST(request: Request) {
  try {
    ...
    const rateLimitError = await rejectRateLimited({
      scope: "provider_health_check",
      request,
      identifierParts: [userId],
      limit: 120,
      windowSeconds: 3600,
      message: "Demasiados chequeos tecnicos en poco tiempo.",
    });
    if (rateLimitError) {
    ...
    const result = await runProviderHealthCheck(contact);
    return NextResponse.json({ result });
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "No se pudo ejecutar el chequeo tecnico.",
      },
      { status: 500 }
    );
  }
